  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section className="contact-section" id="contact" aria-labelledby="contact-title"><div className="container contact-layout"><div><p className="section-number">{t(content.sectionKickers.contact)}</p><h2 id="contact-title">{t(content.contact.title)}</h2><p>{t(content.contact.description)}</p></div><div className="contact-actions"><a className="telegram-card" href={content.contact.primary.url} target="_blank" rel="noreferrer"><TelegramIcon /><div><span>{t(content.contact.primary.responseNote)}</span><strong>{content.contact.primary.value}</strong></div><ExternalArrowIcon /></a><div className="social-grid">{content.contact.socials.map((item) => { const Icon = socialIcons[item.kind]; return <a key={item.kind} className="social-card" href={item.url} target={item.kind === 'email' ? undefined : '_blank'} rel={item.kind === 'email' ? undefined : 'noreferrer'}><Icon /><span>{t(item.label)}</span><strong>{item.value}</strong></a> })}</div></div></div></section>
      </main>
